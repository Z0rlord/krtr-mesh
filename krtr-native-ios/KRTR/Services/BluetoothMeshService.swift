...
class BluetoothMeshService: NSObject {
    ...
    private let collectionsQueue = DispatchQueue(label: "krtr.collections", attributes: .concurrent)
    ...
    weak var delegate: KRTRDelegate?
    ...
    private let messageQueue = DispatchQueue(label: "krtr.messageQueue", attributes: .concurrent) // Concurrent queue with barriers
    ...
    // Message aggregation
    private var pendingMessages: [(message: KRTRPacket, destination: String?)] = []
    ...
    private func flushPendingMessages() {
        ...
        messageQueue.async { [weak self] in
            ...
            var messagesByDestination: [String?: [KRTRPacket]] = [:]
    ...
    private func cacheMessage(_ packet: KRTRPacket, messageID: String) {
    ...
    private func broadcastPacket(_ packet: KRTRPacket) {
        ...
        guard let data = packet.toBinaryData() else { 
            ...
            if senderID == self.myPeerID,
               ...
               let message = KRTRMessage.fromBinaryPayload(packet.payload) {
    ...
    private func handleReceivedPacket(_ packet: KRTRPacket, from peerID: String, peripheral: CBPeripheral? = nil) {
        messageQueue.async(flags: .barrier) { [weak self] in
            guard let self = self else { return }
        ...
        case .message:
            ...
            if let recipientID = packet.recipientID {
                if recipientID == SpecialRecipients.broadcast {
                    ...
                    if let message = KRTRMessage.fromBinaryPayload(packet.payload) {
                        ...
                        let messageWithPeerID = KRTRMessage(
                    ...
                    // Parse the message
                    if let message = KRTRMessage.fromBinaryPayload(decryptedPayload) {
    ...
    private func sendFragmentedPacket(_ packet: KRTRPacket) {
    ...
    private func handleFragment(_ packet: KRTRPacket, from peerID: String) {
        // Handling fragment
        ...
        if let fragments = incomingFragments[fragmentID],
           fragments.count == total {
            ...
            if let reassembledPacket = KRTRPacket.from(reassembledData) {
                // Clean up
...
extension BluetoothMeshService: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        guard let packet = KRTRPacket.from(data) else { 
            return 
        }
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
        for request in requests {
            if let data = request.value {
                ...
                if let packet = KRTRPacket.from(data) {
    ...
    private func handleNoiseEncryptedMessage(from peerID: String, encryptedData: Data, originalPacket: KRTRPacket) {
        // Use noiseService directly
        ...
        do {
            ...
            if let innerPacket = KRTRPacket.from(decryptedData) {
